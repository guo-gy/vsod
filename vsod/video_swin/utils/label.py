...
class CALayer_label(nn.Module):
    def __init__(self, channel, reduction=16):
        super(CALayer_label, self).__init__()
...
class RCAB_label(nn.Module):
    def __init__(
        self, n_feat, kernel_size=3, reduction=16,
        bias=True, bn=False, act=nn.ReLU(True), res_scale=1):

        super(RCAB_label, self).__init__()
        modules_body = []
        for i in range(2):
            modules_body.append(self.default_conv(n_feat, n_feat, kernel_size, bias=bias))
            if bn: modules_body.append(nn.BatchNorm2d(n_feat))
            if i == 0: modules_body.append(act)
        modules_body.append(CALayer_label(n_feat, reduction))
...
class Label(nn.Module):
    ...
    def __init__(self, in_fea=[128, 256, 512, 1024], mid_fea=32):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(in_fea[3], mid_fea, 1),
                                   nn.ReLU(),
                                   nn.Conv2d(mid_fea, mid_fea, 3, padding=1),
                                   nn.ReLU())
        ...
        self.rcab = RCAB_label(mid_fea * 4)
    ...
    def forward(self, x1, x2, x3,x4):
        ...
        edge4 = F.interpolate(x4_fea, scale_factor=32, mode='bilinear', align_corners=True)
