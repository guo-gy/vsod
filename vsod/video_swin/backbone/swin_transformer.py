...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            x = blk(x, attn_mask)
        x = x.view(B, D, H, W, -1)
        out_x = rearrange(x, 'b d h w c -> b c d h w')
        if self.downsample is not None:
            x = self.downsample(x)
...
class SwinTransformer3D(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)):
            out_x, x = self.layers[i](x.contiguous())

            if i==len(self.layers)-1:
                out_x = rearrange(out_x, 'n c d h w -> n d h w c')
                out_x = self.norm(out_x)
                out_x = rearrange(out_x, 'n d h w c -> n c d h w')
            outs.append(out_x)
